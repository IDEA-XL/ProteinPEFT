...
from ..esm_mamba_hf import (
    EsmMambaConfig, 
    EsmTokenizer, 
    EsmMambaForMaskedLM, 
    EsmMambaForSequenceClassification,
)
from ..abstract_model import AbstractModel


class EsmMambaBaseModel(AbstractModel):
    """
    ESM Mamba base model. It cannot be used directly but provides model initialization for downstream tasks.
    """
    ...
    def initialize_model(self):
        ...
        config = EsmMambaConfig.from_pretrained(self.config_path)
        ...
        if self.task == 'classification':
            ...
            if self.load_pretrained:
                self.model = EsmMambaForSequenceClassification.from_pretrained(
                    self.config_path, num_labels=self.num_labels, **self.extra_config)

            else:
                config.num_labels = self.num_labels
                self.model = EsmMambaForSequenceClassification(config)
