...
from .base import EsmMambaBaseModel


@register_model
class EsmMambaAnnotationModel(EsmMambaBaseModel):
    def __init__(self, anno_type: str, **kwargs):
        """
        Args:
            anno_type: one of EC, GO, GO_MF, GO_CC
            **kwargs: other parameters for EsmMambaBaseModel
        """
    ...
    def validation_epoch_end(self, outputs):
        ...
        if not getattr(self, "save_condition", None):
            self.check_save_condition(f1_max, mode="max")
        else:
            for save_condition in getattr(self, "save_condition"):
                self.check_save_condition(log_dict[save_condition], mode="max", condition=save_condition)